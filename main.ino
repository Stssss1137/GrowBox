...
#define FIREBASE_HOST "XXXXXXXXXXXXXXXXXXXXXXXXXXX.firebaseio.com"
...
unsigned long lastWebLogTime = 0;

String dynamicAdmins = "";
String dynamicUsers = "";
...
void loop() {
  ...
  if (millis() - lastUpdate > 3000) {
    ...
    getFirebaseControls();
    checkWebSecurityLogs();
    applyControls();
    updateLCD();
  }
}

void checkWebSecurityLogs() {
  if (Firebase.getInt(fbData, "/growbox/logs/latest_action/timestamp")) {
    unsigned long webTime = fbData.intData();
    if (webTime != lastWebLogTime && lastWebLogTime != 0) {
      if (Firebase.getString(fbData, "/growbox/logs/latest_action/msg")) {
         String webMessage = fbData.stringData();
         broadcastToAdmins(" ЛОГ:\n" + webMessage);
      }
    }
    lastWebLogTime = webTime;
  }
}

void readSensors() {
  sensors_event_t h, t;
  if(aht.getEvent(&h, &t)) { temp = t.temperature; hum = h.relative_humidity; ahtError = false; } 
  else { temp = -99; hum = -99; ahtError = true; }
  
  int rawSoil = analogRead(PIN_SOIL);
  if (rawSoil >= 4090) { soilPct = -1; soilError = true; } 
  else { soilPct = constrain(map(rawSoil, 4095, 1500, 0, 100), 0, 100); soilError = false; }
...
void checkSensorErrors() {
  if (soilError && !lastSoilErrorState) {
    broadcastToAdmins("Гігрометр вірогідно від'єднано");
    ...
    broadcastToAdmins("Гігрометр під'єднано");
...
void getFirebaseControls() {
  // Читаємо права доступу з бази
  if (Firebase.getString(fbData, "/growbox/security/admins")) dynamicAdmins = fbData.stringData();
  if (Firebase.getString(fbData, "/growbox/security/users")) dynamicUsers = fbData.stringData();
  ...
  if (requestedPump && !pumpOn && !soilError) { pumpOn = true; pumpStartTime = millis(); } 
  else if (!requestedPump && pumpOn) { pumpOn = false; }
  ...
  if (currentMode == "AUTO") {
    if (!soilError) {
      if (!pumpOn && soilPct < soilTarget && (millis() - lastPumpRunTime > PUMP_COOLDOWN)) {
        ...
        broadcastToAll("💦Авто-полив 4 сек.");
      }
    } else {
      pumpOn = false; 
    }
...
void updateLCD() {
  ...
  lcd.setCursor(0,3);
  if (pumpOn) lcd.printf("P:1 F:0(ECO) L:D   ");
  else lcd.printf("P:0 F:%d L:%d      ", fanOn, lightOn);
}
